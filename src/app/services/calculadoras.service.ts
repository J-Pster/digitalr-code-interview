...
function calculateValues(calcs: Calc[], form: any): { [id: string]: number } {
  ...
  const typeHandlers: { [type: string]: (items: any[], id: string) => number } = {
    ...
    sub: (items: any[]) => {
      const [a, b] = items.map((item: any) => (typeof item === 'string' ? values[item] : item));
    ...
    'div-rest': (items: any[], id) => {
      ...
      if (typeof dividendId === 'object') {
        ...
        dividend = typeof dividendId === 'string' ? values[dividendId] : dividendId;
      }

      const divisor = typeof divisorId === 'string' ? values[divisorId] : divisorId;
